...
// index: _freelist对应索引
// size:  申请内存大小对齐大小计算，向上取整后的size。申请的总内存
void* ThreadCache::FetchFromCentralCache(size_t index, size_t size)
{
	...
	if (batchsize > 1)
	{
		freelist->PushRange(NEXT_OBJ(start), end, batchsize - 1);//为什么是NEXT_OBJ(start)，因为start 被申请出去了啊
	}
...
//释放对象时，链表过长时，回收内存回到中心缓存; size: 单个内存块的大小
